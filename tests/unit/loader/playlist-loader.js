...
http://proxy-62.dailymotion.com/sec(3ae40f708f79ca9471f52b86da76a3a8)/video/107/282/158282701_mp4_h264_aac_hq.m3u8#cell=core`;
    ...
    let result = new PlaylistLoader({ on: function() { } }).parseMasterPlaylist(manifest, 'http://www.dailymotion.com');
...
http://proxy-21.dailymotion.com/sec(2a991e17f08fcd94f95637a6dd718ddd)/video/107/282/158282701_mp4_h264_aac_fhd.m3u8#cell=core`;
  ...
  it('parses empty levels returns empty fragment array', () => {
    let level = '';
    let result = new PlaylistLoader({ on: function() { } }).parseLevelPlaylist(level, 'http://proxy-62.dailymotion.com/sec(3ae40f708f79ca9471f52b86da76a3a8)/video/107/282/158282701_mp4_h264_aac_hq.m3u8#cell=core', 0);
...
1
#EXT-X-ENDLIST`;
    let result = new PlaylistLoader({ on: function() { } }).parseLevelPlaylist(level, 'http://proxy-62.dailymotion.com/sec(3ae40f708f79ca9471f52b86da76a3a8)/frag(5)/video/107/282/158282701_mp4_h264_aac_hq.m3u8#cell=core', 0);
...
#EXT-X-ENDLIST`;
    let result = new PlaylistLoader({ on: function() { } }).parseLevelPlaylist(level, 'http://foo.com/adaptive/oceans_aes/oceans_aes.m3u8', 0);
...
lo008ts`;
    ...
    let result = new PlaylistLoader({ on: function() { } }).parseLevelPlaylist(level, 'http://dummy.com/playlist.m3u8', 0);
...
#EXT-X-ENDLIST
    `;
    let result = new PlaylistLoader({ on: function() { } }).parseLevelPlaylist(level, 'http://video.example.com/disc.m3u8', 0);
    assert.strictEqual(result.fragments.length, 5);
...
#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID="600k",LANGUAGE="eng",NAME="Audio",AUTOSELECT=YES,DEFAULT=YES,URI="/videos/ZakEbrahim_2014/audio/600k.m3u8?qr=true&preroll=Blank",BANDWIDTH=614400`;
    let result = new PlaylistLoader({ on: function() { } }).parseMasterPlaylistMedia(manifest, 'https://hls.ted.com/', 'AUDIO');
    assert.strictEqual(result.length, 1);
...
http://dummy.url.com/hls/live/segment/segment_022916_164500865_719935.ts`;
    let result = new PlaylistLoader({ on: function () { } }).parseLevelPlaylist(level, 'http://dummy.url.com/playlist.m3u8', 0);
    assert.strictEqual(result.fragments.length, 10);
